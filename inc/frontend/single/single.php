...
get_template_part('inc/frontend/single/subheader');
...
<div id="single_wrapper">
    ...
    if (have_posts()) :
        while (have_posts()) : the_post();
            ?>
            <?php
            $total_imgs = count($postImageIds) + count($hotel_images_ids);
            if ($total_imgs > 0){
                ?>
                <div class="images-list">
                <?php
                $img_margin = $total_imgs-1;
                $img_width = 100/$total_imgs;

                foreach ($postImageIds as $key => $imgId) {
                    $img_small = wp_get_attachment_url($imgId);
                    $backgroundStyle = "background-image: url('" . $img_small . "'); width: calc(".$img_width."% - 2px);";
                    ?>
                    <div class="single-post-img" style ="<?php echo $backgroundStyle; ?>"><a data-gallery="multiimages" data-toggle="lightbox" href="<?php echo $img_small;?>" class="single-image" ></a></div>
                <?php }
                if($hotel_images_ids){ ?>
                    <?php foreach ($hotel_images_ids as $key => $hotel_image_id):
                        $img_small = wp_get_attachment_url($hotel_image_id);
                        $backgroundStyle = "background-image: url('" . $img_small . "'); width: calc(".$img_width."% - 2px);";?>
                        <div class="single-post-img" style ="<?php echo $backgroundStyle; ?>"><a data-gallery="multiimages" data-toggle="lightbox" href="<?php echo $img_small;?>" class="single-image" ></a></div>
                    <?php endforeach; ?>
                <?php
            ...
            if($place_id){
                get_template_part("inc/frontend/map/single-map");
            }
            ?>
            ...
            if ($show_related == '1' || $show_related == 'yes') :
                ...
                <div class="ts-related-posts-on-single ts-post-section-inner">
                    <div class="header-section">
                        <div class="container">
                            <p class="module-header"><?php echo ts_option_vs_default('related_blog_posts_title_text', 'Related Posts'); ?></p>
                        </div>
                    </div>
            ...
            ?>
                <div class="ts-related-posts-on-single ts-post-section-inner">
                    <div class="header-section">
                        <div class="container">
                            <p class="module-header">More Jrrnys by this Contributor</p>
                        </div>
                    </div>
